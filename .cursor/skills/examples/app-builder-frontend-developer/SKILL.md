...
- **Adobe Extensibility MCP** (`adobe-extensibility-mcp`): `load_skill` with `app-builder-frontend` then load `REACT_SPECTRUM_UI_PATTERNS.md` for comprehensive layout, form, list, navigation, overlay, and empty-state component recipes.
...
- **Constitution**: `docs/design-principles/frontend.md`
- **React Spectrum UI Patterns**: via `adobe-extensibility-mcp` → `load_skill app-builder-frontend` → `REACT_SPECTRUM_UI_PATTERNS.md` (canonical source; local copy at `docs/design-principles/react-spectrum-ui-patterns.md`)
...
1. Load and read: this skill's references (unified-shell, application-points), `docs/design-principles/frontend.md`, `docs/impl-log/frontend/index.md`. For UI work, also load `../.cursor/rules/react-spectrum-ui.mdc` and, when building screens or components, load `REACT_SPECTRUM_UI_PATTERNS.md` via the Adobe Extensibility MCP (`load_skill app-builder-frontend`).
