> **FRAMEWORK EXAMPLE**: This file is Adobe App Builder-specific. It demonstrates how to author a platform-specific "application points" reference doc for your domain specialist skills. Replace with the equivalent for your platform or delete if not applicable.
...
| ----------------------------------------------------------- | ----------------------------- | ------------------------------------- | --------------------------------------------------------------------------- |---------------------------------------|
| **Unified shell / Experience shell** (experience.adobe.com) | `src/{{APP_EXTENSION_POINT}}/`          | `src/{{APP_EXTENSION_POINT}}/actions/`          | `src/{{APP_EXTENSION_POINT}}/web-src/`, `src/{{APP_EXTENSION_POINT}}/public/`                   |`docs/developer-setup/app-builder-frontend-unified-shell.md` |
...
## For This Repo ({{PROJECT_NAME}} Publishing)

{{PROJECT_NAME}} extends the **unified/experience shell**. Therefore:

- **Actions** live under `src/{{APP_EXTENSION_POINT}}/actions/`.
- **Frontend** lives under `src/{{APP_EXTENSION_POINT}}/web-src/` and `src/{{APP_EXTENSION_POINT}}/public/`.
...
when launching an application which has more than one extension point defined in the project you must issues the run and dev commands with the -e flag and specify the extension point. IG `aio app run -e {{APP_EXTENSION_POINT}}`
...
my-ai-app/
├── src/
│   ├── {{APP_EXTENSION_POINT}}/         # Unified Shell (if being extended)
...
- [design-principles/architecture.md](../design-principles/architecture.md) — module boundaries for this project
